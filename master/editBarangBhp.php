<?php 
	...
	if (isset($_POST['getSimpan'])) {
		...
		$jenis_barang = $br->validateHtml($_POST['jenis_barang']);
		$distributor  = $br->validateHtml($_POST['distributor']);
		$harga        = $br->validateHtml($_POST['harga']);
		$stok         = $br->validateHtml($_POST['stok']);
		$ket          = $_POST['ket'];

		if ($kode_barang == " " || $nama_barang == " " || $jenis_barang == " " || $distributor == " " || $harga == " " || $stok == " " || $ket == " " ) {
			...
			if ($harga < 0 || $stok < 0) {
				...
				if ($_FILES['foto']['name'] == "") {
					$value = "kd_barang='$kode_barang',nama_barang='$nama_barang',kd_jenisbarang='$jenis_barang',kd_distributor='$distributor',tanggal_masuk='$waktu',harga_barang='$harga',stok_barang='$stok',keterangan='$ket'";
					$response = $br->update($table,$value,"kd_barang",$_GET['id'],"?page=viewBarang");
				}else{
					$response = $br->validateImage();
					if ($response['types'] == "true") {
						$value = "kd_barang='$kode_barang',nama_barang='$nama_barang',kd_jenisbarang='$jenis_barang',kd_distributor='$distributor',tanggal_masuk='$waktu',harga_barang='$harga',stok_barang='$stok',keterangan='$ket',gambar='$response[image]'";
						$response = $br->update($table,$value,"kd_barang",$_GET['id'],"?page=viewBarang");
					}else{
                        				...
                        				<div class="col-md-6">
											...
											<div class="form-group">
												...
												<select name="jenis_barang" class="form-control">
													...
													<?php if ($mr['kd_jenisbarang'] == $data['kd_jenisbarang']){ ?>
														<option value="<?= $mr['kd_jenisbarang'] ?>" selected><?= $mr['jenis_barang'] ?>
														</option>
													<?php }else{ ?>
